...
function ProjectsManagement() {
  ...
  const [formData, setFormData] = useState({
    title: '', description: '', type: 'publication',
    authors: '', year: '', link: '',
    abstract: '', keywords: '', figure_url: '',
    figure_caption: '', publication_info: '',
    citation: '', detail_content: '', related_member_ids: '',
    pdf_url: '', code_url: ''
  })
  ...
  const handleOpenModal = (item = null) => {
    if (item) {
      ...
      setFormData({
        ...
        type: item.type, authors: item.authors || '',
        year: item.year || '', link: item.link || '',
        abstract: item.abstract || '', keywords: item.keywords || '',
        figure_url: item.figure_url || '', figure_caption: item.figure_caption || '',
        publication_info: item.publication_info || '',
        citation: item.citation || '', detail_content: item.detail_content || '',
        related_member_ids: item.related_member_ids || '',
        pdf_url: item.pdf_url || '', code_url: item.code_url || ''
      })
    } else {
      setEditingItem(null)
  ...
  const getImageArray = (url) => {
    if (!url) return []
    try { return JSON.parse(url) } catch { return url ? [url] : [] }
  }
  ...
  const handleDelete = async (id) => {
    ...
    catch (error) { console.error('Failed to delete project:', error); alert('删除失败') }
  }

  const getTypeLabel = (type) => {
    const labels = { publication: '学术论文', project: '研究项目', teaching: '专业课程' }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h2>学术成果管理</h2>
        <button className="btn btn-primary" onClick={() => handleOpenModal()}>添加成果</button>
      </div>

      <div className="card">
        <table className="table">
          <thead>
            <tr><th>标题</th><th>类型</th><th>作者/负责人</th><th>年份</th><th>操作</th></tr>
          </thead>
          <tbody>
            {projects.map(project => (
              <tr key={project.id}>
                ...
                <td>{project.year || '-'}</td>
                <td>
                  <button className="btn btn-secondary" onClick={() => handleOpenModal(project)} style={{ marginRight: '10px' }}>编辑</button>
                  <button className="btn btn-danger" onClick={() => handleDelete(project.id)}>删除</button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
          <div className="card" style={{ width: '650px', maxHeight: '85vh', overflow: 'auto' }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>类型 *</label>
                <select className="input" value={formData.type} onChange={(e) => setFormData({...formData, type: e.target.value})}>
                  <option value="publication">学术论文</option>
                  <option value="project">研究项目</option>
                  <option value="teaching">专业课程</option>
                </select>
              </div>
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>作者/负责人</label>
              ...
              <div style={{ display: 'flex', gap: '10px', marginBottom: '15px' }}>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>年份</label>
                ...
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>链接 (DOI等)</label>
                  <input type="url" className="input" value={formData.link} onChange={(e) => setFormData({...formData, link: e.target.value})} placeholder="https://..." />
                </div>
              </div>
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>出版信息</label>
              ...
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>摘要</label>
                <textarea className="input" rows="6" value={formData.abstract} onChange={(e) => setFormData({...formData, abstract: e.target.value})} />
              </div>
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>关键词</label>
              ...
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>论文配图（可上传多张）</label>
                {getImageArray(formData.figure_url).map((url, idx) => (
                  <div key={idx} style={{ position: 'relative', display: 'inline-block', marginRight: '8px', marginBottom: '8px' }}>
                    ...
                    <button type="button" onClick={() => removeFigureImage(idx)}
                      style={{ position: 'absolute', top: '-6px', right: '-6px', background: '#c1272d', color: '#fff',
                        border: 'none', borderRadius: '50%', width: '20px', height: '20px', fontSize: '12px',
                        cursor: 'pointer', lineHeight: '20px', textAlign: 'center' }}>×</button>
                  </div>
                ))}
                ...
                <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                  <input id="manual-figure-url" type="text" className="input"
                    placeholder="或输入图片URL后点击添加" style={{ flex: 1 }} />
                  <button type="button" className="btn btn-secondary" onClick={addFigureUrl}>添加</button>
                </div>
              </div>
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>配图说明</label>
              ...
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>PDF 文件</label>
                {formData.pdf_url && (
                  <div style={{ marginBottom: '8px' }}>
                    <span style={{ fontSize: '13px', color: '#1976d2' }}>已上传: {formData.pdf_url.split('/').pop()}</span>
                    <button type="button" onClick={() => setFormData({...formData, pdf_url: ''})}
                      style={{ marginLeft: '8px', background: '#c1272d', color: '#fff', border: 'none', borderRadius: '50%', width: '18px', height: '18px', fontSize: '11px', cursor: 'pointer' }}>×</button>
                  </div>
                )}
                ...
                {uploadingPdf && <span style={{ marginLeft: '10px', fontSize: '13px' }}>上传中...</span>}
              </div>
              <div style={{ display: 'flex', gap: '10px', marginBottom: '15px' }}>
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>Code 网址</label>
                ...
                <div style={{ flex: 1 }}>
                  <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>DOI 网址</label>
              ...
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>引用格式</label>
                <textarea className="input" rows="3" value={formData.citation} onChange={(e) => setFormData({...formData, citation: e.target.value})} />
              </div>
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>详细内容 (研究背景、方法、结果等)</label>
              ...
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', marginBottom: '5px', fontSize: '14px' }}>关联成员</label>
                <div style={{ maxHeight: '120px', overflow: 'auto', border: '1px solid #ddd', borderRadius: '4px', padding: '8px' }}>
                  {allMembers.map(m => {
                    ...
                    return (
                      <label key={m.id} style={{ display: 'inline-block', marginRight: '12px', marginBottom: '4px', fontSize: '13px', cursor: 'pointer' }}>
