...
function NewsManagement() {
  ...
  const getRelatedPapers = () => {
    ...
    try { return JSON.parse(formData.related_project_ids) } catch { return [] }
  }
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ marginBottom: '15px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                  <input id="manual-image-url" type="text" className="input"
              ...
              <div style={{ marginBottom: '15px' }}>
                ...
                {getRelatedPapers().map((paper, idx) => (
                  <div key={idx} style={{ border: '1px solid #ddd', borderRadius: '4px', padding: '10px', marginBottom: '8px', position: 'relative' }}>
                    <button type="button" onClick={() => removeRelatedPaper(idx)}
                      style={{ position: 'absolute', top: '4px', right: '8px', background: '#c1272d', color: '#fff',
                        border: 'none', borderRadius: '50%', width: '18px', height: '18px', fontSize: '11px', cursor: 'pointer' }}>×</button>
                    <input type="text" className="input" placeholder="论文标题" value={paper.title}
                      onChange={(e) => updateRelatedPaper(idx, 'title', e.target.value)}
                      style={{ marginBottom: '6px', fontSize: '13px' }} />
                    <div style={{ display: 'flex', gap: '6px', marginBottom: '6px' }}>
                      <input type="text" className="input" placeholder="作者" value={paper.authors}
                        onChange={(e) => updateRelatedPaper(idx, 'authors', e.target.value)} style={{ flex: 1, fontSize: '13px' }} />
                      <input type="text" className="input" placeholder="日期" value={paper.date}
                        onChange={(e) => updateRelatedPaper(idx, 'date', e.target.value)} style={{ width: '120px', fontSize: '13px' }} />
                    </div>
                    <input type="url" className="input" placeholder="DOI链接 (可选)" value={paper.link}
                      onChange={(e) => updateRelatedPaper(idx, 'link', e.target.value)} style={{ fontSize: '13px' }} />
                  </div>
                ))}
                <button type="button" className="btn btn-secondary" onClick={addRelatedPaper}
                  style={{ fontSize: '13px' }}>+ 添加论文</button>
              </div>
